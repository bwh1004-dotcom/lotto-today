...
function LocationScreen({onSave,onShare,onBack}){
  ...
  useEffect(()=>{
    ...
    const drawBg=()=>{
      ...
      sky.addColorStop(0.5,"#030816");
      sky.addColorStop(1,"#05122a");
      ctx.fillStyle=sky;
      ctx.fillRect(0,0,W,H*0.72);

      // 별
      for(let i=0;i<60;i++){
        const sx=(i*137+23)%W, sy=(i*97+11)%(H*0.6);
        ctx.fillStyle=`rgba(255,255,255,${0.2+((i*7)%10)*0.06})`;
        ctx.beginPath();
        ctx.arc(sx,sy,i%5===0?1.2:0.6,0,Math.PI*2);
        ctx.fill();
      }

      // 바다
      const sea=ctx.createLinearGradient(0,H*0.7,0,H);
      sea.addColorStop(0,"#0a1e3a");
      sea.addColorStop(1,"#05101e");
      ctx.fillStyle=sea;
      ctx.fillRect(0,H*0.7,W,H*0.3);

      // 물결
      for(let i=0;i<8;i++){
        ctx.strokeStyle=`rgba(105,200,242,${0.04+i*0.01})`;
        ctx.lineWidth=1;
        ctx.beginPath();
        const y=H*0.73+i*(H*0.03);
        ctx.moveTo(0,y);
        for(let x=0;x<W;x+=8){
      ...
      for(let i=0;i<7;i++){
        const px=(W/6)*i;
        ctx.fillRect(px-3,BH-40,6,44);
        // 케이블 타워
        ctx.fillRect(px-2,BH-60,4,22);
      }
      // 주 케이블
      ...
      for(let i=0;i<7;i++){
        const px=(W/6)*i;
        ctx.quadraticCurveTo(px-W/12,BH-30,px,BH-60);
        ctx.quadraticCurveTo(px+W/12,BH-30,(W/6)*(i+1),BH-50);
      }
      ctx.stroke();
      // 상판
      ctx.fillStyle="#0d1e36";
      ctx.fillRect(0,BH-4,W,6);
      // 다리 불빛
      for(let i=0;i<W/10;i++){
        const brightness=Math.random()>0.5;
        ctx.fillStyle=brightness?"rgba(249,215,28,0.5)":"rgba(249,215,28,0.2)";
        ctx.beginPath();
        ctx.arc(i*10,BH-2,1.5,0,Math.PI*2);
        ctx.fill();
      }

      // 수면 반사
      const ref=ctx.createLinearGradient(0,BH,0,H);
      ref.addColorStop(0,"rgba(13,30,54,0.7)");
      ref.addColorStop(1,"rgba(5,16,30,0.95)");
      ctx.fillStyle=ref;
      ctx.fillRect(0,BH,W,H-BH);

      // 반사 불빛
      for(let i=0;i<W/10;i++){
        ctx.fillStyle="rgba(249,215,28,0.1)";
        ctx.fillRect(i*10-1,BH+2,2,H*0.05);
      }
    };
  ...
  const spawnFirework=(e)=>{
    if(taps>=MAX_TAPS||phase!=="play")return;
    const canvas=canvasRef.current;
    if(!canvas)return;
    ...
    if(cy>canvas.height*0.70)return;

    const pal=[["#f9d71c","#ffed4a"],["#ff4444","#ff8888"],["#69c8f2","#a0d8ef"],["#b0d840","#d4f17a"],["#ff9800","#ffcc02"]][Math.floor(Math.random()*5)];
    for(let i=0;i<80;i++){
      const a=Math.random()*Math.PI*2,s=1+Math.random()*4;
      const life=80+Math.floor(Math.random()*70);
      particles.current.push({x:cx,y:cy,vx:Math.cos(a)*s*0.7,vy:Math.sin(a)*s*0.7-0.8,
        color:pal[Math.floor(Math.random()*pal.length)],r:2+Math.random()*3,life,maxLife:life});
    }
    const num=Math.floor(Math.random()*8)+(taps*7+3)%38+1;
    const bounded=Math.max(1,Math.min(45,num));
    tapNums.current=[...tapNums.current,bounded];
    const newTaps=taps+1;
    setTaps(newTaps);
    if(newTaps>=MAX_TAPS){
      setTimeout(()=>{
        if(alive.current){setNums(pickNums(tapNums.current));setPhase("result");}
      },1500);
    }
  };

  const reset=()=>{taps && setTaps(0);setNums([]);setPhase("play");tapNums.current=[];particles.current=[];};

  const W2={minHeight:"100%",background:"#010208",display:"flex",flexDirection:"column",
    alignItems:"center",justifyContent:"flex-start",padding:"10px 14px",boxSizing:"border-box",color:"#fff"};

  return(<div style={W2}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%",maxWidth:390,marginBottom:8}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:"#666",fontSize:13,cursor:"pointer"}}>← 홈</button>
      <span style={{fontSize:13,fontWeight:900,color:"#f9d71c"}}>🎆 광안대교 불꽃</span>
      <span style={{fontSize:11,color:"#555"}}>{taps}/{MAX_TAPS}</span>
    </div>

    {/* 캔버스 */}
    <canvas ref={canvasRef} width={390} height={260}
      style={{width:"100%",maxWidth:390,borderRadius:14,cursor:phase==="play"?"crosshair":"default",
        touchAction:"none",boxShadow:"0 4px 24px rgba(0,0,0,0.6)"}}
      onTouchStart={e=>{e.preventDefault();spawnFirework(e);}}
      onMouseDown={e=>{if(e.pointerType==="touch")return;spawnFirework(e);}}/>

    <div style={{fontSize:11,color:"#555",marginTop:6,textAlign:"center"}}>
      {phase==="play"&&taps<MAX_TAPS&&"다리 위 하늘을 탭해서 불꽃을 쏘세요 🎆"}
      {phase==="play"&&taps>=MAX_TAPS&&"🎆 마지막 불꽃! 번호 계산 중..."}
      {phase==="result"&&"🎆 오늘 밤 광안대교의 불꽃"}
    </div>

    {phase==="result"&&(<div style={{marginTop:12,width:"100%",maxWidth:390,textAlign:"center"}}>
      <Balls nums={nums}/><NumLine nums={nums}/>
      <ActionBtns nums={nums} mode="🎆 불꽃놀이" onSave={onSave} onShare={onShare}/>
      <button onClick={reset} style={{marginTop:10,background:"linear-gradient(135deg,#c8a800,#f9d71c)",border:"none",borderRadius:50,padding:"11px 24px",fontSize:13,fontWeight:900,color:"#000",cursor:"pointer"}}>다시 쏘기 🎆</button>
    </div>)}
  </div>);
}
